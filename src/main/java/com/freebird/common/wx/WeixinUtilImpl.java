...
import com.threeguys.common.http.BusinessResponse;
import com.threeguys.common.http.NetUtil;
import com.threeguys.common.http.WeixinHttpURLSet;
import com.threeguys.common.util.LogUtils;
import com.threeguys.common.util.RedisClientUtil;
...
//			String scope = jsonObject.getString("scope");
			...
			if (response.getRt_code()==BusinessResponse.RESPONSE_OK){
				Map<String,String> data = gson.fromJson(response.getData().toString(), new TypeToken<Map<String,String>>(){}.getType());
	...
	private synchronized void refreshToken() {
		String idByte = this.getTokenFromRedis();
		AccessToken token = JSONObject.parseObject(idByte,AccessToken.class);
		//再次判断防止并发问题
		if(token.needRefresh()){
			try {
				long now = System.currentTimeMillis();
...
//				JSONObject result = HttpRequestUtil.httpsGETRequest(getAccessTokenURL);
	...
	private void setTokenToRedis(AccessToken token) {
		RedisClientUtil.setString(("accessToken_" + this.appId), JSONObject.toJSONString(token));
	}
...
//		JSONObject ticketObj = HttpRequestUtil.httpsGETRequest(url_getticket);
	...
	public Map<String, Object> getBaseUserInfoByAuth(String auth_access_token, String openid) {
		...
		if (response.getRt_code()==BusinessResponse.RESPONSE_OK) {
			data = gson.fromJson(response.getData().toString(), new TypeToken<Map<String, Object>>() {}.getType());
			//token已失效
